...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    , RedMiddleLeft, RedMiddleRight, RedAbove, RedAboveLeft, RedAboveRight, RedBelow, RedBelowLeft, RedBelowRight
    // This one of the ways to dynamilcy create/allocate two dimentional array
    // This creats a tow dimentional array calles "image_temp[][]" it is excalty the same size as "image[][]"
    RGBTRIPLE(*image_temp)[width] = calloc(height, width * sizeof(RGBTRIPLE));
           ...
           if (i == 0 && m == 0)
            {
                ...
                Gx_Value = round((RedMiddle * 0 + RedAboveLeft * -1 + RedAbove * 0 + RedAboveRight * 1 + RedMiddleLeft * -2 + RedMiddleRight
                * 2 + RedBelowLeft * -1 + RedBelow * 0 + RedBelowRight * 1));
                Gy_Value = round((RedMiddle * 0 + RedAboveLeft * -1 + RedAbove * -2 + RedAboveRight * -1 + RedMiddleLeft * 0 + RedMiddleRight
                * 0 + RedBelowLeft * 1 + RedBelow * 2 + RedBelowRight * 1));
                image_temp[i][m].rgbtRed = sqrt(Gx_Value * Gx_Value + Gy_Value * Gy_Value);
                if (image_temp[i][m].rgbtRed > 255)
                {
                    image_temp[i][m].rgbtRed = 255;
                }

               Gx_Value = round((GreenMiddle * 0 + GreenAboveLeft * -1 + GreenAbove * 0 + GreenAboveRight * 1 + GreenMiddleLeft * -2 + GreenMiddleRight
                * 2 + GreenBelowLeft * -1 + GreenBelow * 0 + GreenBelowRight * 1));
                Gy_Value = round((GreenMiddle * 0 + GreenAboveLeft * -1 + GreenAbove * -2 + GreenAboveRight * -1 + GreenMiddleLeft * 0 + GreenMiddleRight
                * 0 + GreenBelowLeft * 1 + GreenBelow * 2 + GreenBelowRight * 1));
                image_temp[i][m].rgbtGreen = sqrt(Gx_Value * Gx_Value + Gy_Value * Gy_Value);
                if (image_temp[i][m].rgbtGreen > 255)
                {
                    image_temp[i][m].rgbtGreen = 255;
                }

                Gx_Value = round((BlueMiddle * 0 + BlueAboveLeft * -1 + BlueAbove * 0 + BlueAboveRight * 1 + BlueMiddleLeft * -2 + BlueMiddleRight
                * 2 + BlueBelowLeft * -1 + BlueBelow * 0 + BlueBelowRight * 1));
                Gy_Value = round((BlueMiddle * 0 + GreenAboveLeft * -1 + BlueAbove * -2 + BlueAboveRight * -1 + BlueMiddleLeft * 0 + BlueMiddleRight
                * 0 + BlueBelowLeft * 1 + BlueBelow * 2 + BlueBelowRight * 1));
                image_temp[i][m].rgbtBlue = sqrt(Gx_Value * Gx_Value + Gy_Value * Gy_Value);
                if (image_temp[i][m].rgbtRed > 255)
                {
                    image_temp[i][m].rgbtGreen = 255;
                }
